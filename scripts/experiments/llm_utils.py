...
# --- Configuracion sensible cargada desde variables de entorno ---
# NUNCA hardcodees claves aqui. Define estas variables en tu shell o en un
# archivo .env (ignorado por git). Ejemplo:
...
wandb_project_name = os.environ.get(
    "WANDB_PROJECT",
    "Optuna-unsloth-DeepSeek-R1-Distill-Llama-8B-unsloth-bnb-4bit-20-eval-examples_space_auto_sampler",
)

TOKEN   = os.environ.get("TELEGRAM_BOT_TOKEN")
CHAT_ID = os.environ.get("TELEGRAM_CHAT_ID")

os.environ["WANDB_ENTITY"] = os.environ.get("WANDB_ENTITY", "ipachecoc1600-instituto-polit-cnico-nacional")
...
class LLMModel:
    ...
    def __init__(self, model_name):
        ...
        self.stratified_folds   = None

        #For trainer
        self.trainer_train_dataset = None
        self.trainer_eval_dataset  = None
        
        self.STUDY_PKL              = f"{wandb_project_name}_study.pkl"
    ...
    def generate_model_output(self, propmt , max_new_tokens=1024, temperature=1.0, top_k=50, top_p=1.0, do_sample=False, skip_special_tokens = True,  repetition_penalty=1.0, safety_margin=0.9):
        ...
        try:
            ...
            inputs = self.tokenizer(propmt , return_tensors="pt").to(self.model.device)
    ...
    def perform_stratified_cv(self, df, n_splits=10, random_state=42):
        try:
            # X e y son correctos para el split
            X = df.drop(columns=['[score]', '[COT]', "[num_annotators]", "[holistic_scores_per_annotator]"] + [f'[R{i}]' for i in range(1, 8)])
            y = df['[score]']
            
            skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=random_state)
            
            # Guardaremos tuplas (train_df, val_df)
            ...
            for train_index, val_index in skf.split(X, y):
                ...
                self.stratified_folds.append((train_df, val_df))
                
            print(f"Se crearon exitosamente {len(self.stratified_folds)} folds.")
    ...
    def n_shot_experiment(self, promt, max_new_tokens = 1024, n_shot=0, show_chart = False, save_path = None, verbose = False):
        ...
        try:
            ...
            for index, row in self.test_dataset.iterrows():
                ...
                if n_shot > 0:
                    fs_promt = self.build_few_shot_prompt(promt, n_shot_examples, text)
                else:
                    fs_promt = promt.format(text, "")

                if len(fs_promt) > greater_prompt_length:
                    greater_prompt_length = len(fs_promt)
                    print(f"Greater prompt length: {greater_prompt_length}")

                gen_text = self.generate_model_output(fs_promt, skip_special_tokens=False, max_new_tokens=max_new_tokens)
    ...
    def format_prompt_func(self, user_prompt, assistant_prompt, row, EOS_TOKEN):
        ...
        message = [
            ...
            {
                "role": "assistant",
                "content": assistant_prompt.format(row['[COT]'], output)
            }
        ]
    ...
    def load_or_create_study(self, sampler=None, previous_data=None):
        ...
        if previous_data is not None:
            for trial_info in previous_data:
                ...
                values=[f1_score],
                distributions={
                    'r': IntDistribution(high=32, log=False, low=8, step=2), 
                    'lora_alpha': IntDistribution(high=64, log=False, low=4, step=4), 
    ...
    class CoherenceEvalCallback(TrainerCallback):
        ...
        def on_evaluate(self, args, state, control, metrics=None, **kwargs):
            ...
            with torch.no_grad():
                ...
                k = min(self.num_samples or len(self.eval_dataset)//2,
                len(self.eval_dataset)//2)
                neg = self.eval_dataset.filter(lambda x: x["[score]"] == 0).shuffle(seed=SEED).select(range(k))
                pos = self.eval_dataset.filter(lambda x: x["[score]"] == 1).shuffle(seed=SEED).select(range(k))
                eval_subset = concatenate_datasets([neg, pos]).shuffle(seed=SEED)
    ...
    def _clean_model_output(self, text):
        """
        Limpia artefactos de tokens BPE y elimina el bloque de pensamiento de DeepSeek.
        """
        if not text:
            return ""
        # 1. Corregir fugas de bytes comunes del tokenizador de Llama/DeepSeek
        text = text.replace('Ġ', ' ').replace('Ċ', '\n').replace('ĉ', '\t').replace('âĢĻ', "'")
        
        # 2. Eliminar por completo el bloque de razonamiento de DeepSeek (<think>...</think>)
    ...
    def evaluate_lora_mode(self, prompt, custom_dataframe=None, batch_size=8, max_new_tokens=None, temperature=1.0, top_k=50, top_p=1.0, do_sample=False, streaming = False, stop_on_first_input = False):
        ...
        try:
            ...
            # Para generacion en modelos decoder-only el padding DEBE ser a la izquierda,
            # de lo contrario los tokens de padding quedan entre el prompt y la generacion
            # y la salida en batch (batch_size > 1) se corrompe.
            self.tokenizer.padding_side = "left"
            if self.tokenizer.pad_token is None:
                self.tokenizer.pad_token = self.tokenizer.eos_token

            # Tokenize inputs in batches (max_length explicito para no truncar la historia
            # ni el marcador final de respuesta de forma silenciosa).
            ...
            # Split into batches
            dataset = list(zip(input_ids, attention_mask, labels, original_texts, stories_ids))
            dataloader = DataLoader(dataset, batch_size=batch_size)

            y_true = []
            y_pred = []
            tp, fp, tn, fn = [], [], [], []
            # Seguimiento de fallos de parseo: historias donde no se pudo extraer
            # un <score> valido. Se reportan aparte para no confundir "no parseable"
            # con "prediccion incorrecta".
            parse_failure_ids = []
            ...
            for batch in tqdm(dataloader, desc="Evaluating"):
                ...
                if stop_on_first_input:
                    ...
                    print(f"IDs: {original_texts_batch}")
                
                print(f"Stories ids: {stories_ids_batch}")
                ...
                # contrario HF emite warnings y pueden alterar el decoding greedy.
                if do_sample:
                ...
                # IMPORTANTE: generate() devuelve prompt + generacion. Si decodificamos
                # todo, get_output_score capturaria los <score> de ejemplo del propio
                # prompt. Nos quedamos SOLO con los tokens nuevos generados.
                gen_tokens = outputs[:, input_ids_batch.shape[1]:]
                decoded = self.tokenizer.batch_decode(gen_tokens, skip_special_tokens=True)
                
                for text, true_score, original_text, story_id in zip(decoded, labels_batch, original_texts_batch, stories_ids_batch):
                    
                    # --- AQUÍ APLICAMOS LA LIMPIEZA ROBUSTA ---
                    cleaned_text = self._clean_model_output(text)
                    pred_score = self.get_output_score(cleaned_text)
                        
                    if pred_score is None:
                        print("*"*50)
                        print(f"Warning: Score is None para Story ID {story_id.item()}. No se pudo parsear <score>.")
                        print("*"*50)
                        # Fallback FIJO e independiente de la etiqueta real para no
                        # sesgar las metricas (antes se forzaba el valor opuesto a y_true,
                        # lo que convertia todo fallo de parseo en un error garantizado).
                        parse_failure_ids.append(story_id.item())
                        pred_score = 0
                    else:
                        # Si el modelo alucina un score fuera de rango (como tu salida con score 10)
                        if pred_score not in [0, 1]:
                            pred_score = 0 if pred_score in [2, 10] else 1
                    
                    print(f"Y_true: {true_score.item()}")
                    print(f"Y_pred: {pred_score}")
                    
                    # Clasificación de métricas
                    t_val = true_score.item()
                    if t_val == 1 and pred_score == 1: tp.append(story_id.item())
                    elif t_val == 0 and pred_score == 1: fp.append(story_id.item())
                    elif t_val == 0 and pred_score == 0: tn.append(story_id.item())
                    elif t_val == 1 and pred_score == 0: fn.append(story_id.item())

                    y_true.append(t_val)
                    y_pred.append(pred_score)
                    
                    result = self.highlight_problematic_sentences(original_text, cleaned_text, score=pred_score)
                    display(result)
                    
                if stop_on_first_input:
                    break

            # Guardamos la info de fallos de parseo del ultimo run para que el
            # llamador (p.ej. cross-validation) la pueda agregar y reportar.
            n_total = len(y_pred)
            n_fail = len(parse_failure_ids)
            self.last_parse_failure_ids = parse_failure_ids
            self.last_parse_failure_count = n_fail
            self.last_parse_failure_rate = (n_fail / n_total) if n_total else 0.0
            print(
                f"Parse failures: {n_fail}/{n_total} "
                f"({self.last_parse_failure_rate:.1%}) -> IDs: {parse_failure_ids}"
            )
    ...
    def run_cross_validation_evaluation(self, prompt, batch_size=8, run_first_fold_only=False, streaming=False):
        """
        Recorre los folds creados y ejecuta la evaluación en cada set de validación.
        """
        all_folds_results = []
        
        # 1. Asegúrate de que los folds existan
        if not hasattr(self, 'stratified_folds') or not self.stratified_folds:
            print("Error: No se han creado los folds. Ejecuta primero perform_stratified_cv")
            return

        # 2. Iterar sobre cada fold
        for i, (train_df, val_df) in enumerate(self.stratified_folds):
            print(f"\n" + "="*50)
            print(f"Iniciando evaluación Fold {i+1} de {len(self.stratified_folds)}")
            print(f"Tamaño del set de validación: {len(val_df)}")
            print("="*50)
            
            # 3. Llamar a tu función de evaluación
            y_true, y_pred = self.evaluate_lora_mode(
                prompt=prompt,
                custom_dataframe=val_df,
                batch_size=batch_size,
                max_new_tokens=1024 * 4,
                # Decodificacion GREEDY y determinista para que los folds sean
                # comparables y reproducibles al medir generalizacion.
                do_sample=False,
                streaming=streaming,
            ...
            all_folds_results.append({
                ...
                'parse_failures': getattr(self, 'last_parse_failure_count', 0),
                'parse_failure_rate': getattr(self, 'last_parse_failure_rate', 0.0),
                'parse_failure_ids': getattr(self, 'last_parse_failure_ids', []),
            })
            
            print(f"Resultados Fold {i+1}:")
            self.show_metrics(y_true, y_pred)
            
            # 5. Lógica para detenerse tras el primer fold si se solicita
            if run_first_fold_only:
                print("\nFlag 'run_first_fold_only' activo. Deteniendo evaluación tras el primer fold.")
                break
                
        print("\nEvaluación completada.")
        self._report_cv_aggregate(all_folds_results)
        return all_folds_results

    def _report_cv_aggregate(self, all_folds_results):
        """Reporta media +/- desviacion estandar de las metricas a traves de los folds."""
        if not all_folds_results:
            print("No hay resultados de folds para agregar.")
            return

        metric_keys = ['f1_macro', 'accuracy', 'precision_macro', 'recall_macro', 'parse_failure_rate']
        print("\n" + "="*50)
        print(f"RESUMEN CROSS-VALIDATION ({len(all_folds_results)} folds)")
        print("="*50)
        for key in metric_keys:
            values = [fold[key] for fold in all_folds_results if key in fold]
            if values:
                print(f"{key:>20}: {np.mean(values):.4f} +/- {np.std(values):.4f}")

        total_failures = sum(fold.get('parse_failures', 0) for fold in all_folds_results)
        total_samples = sum(len(fold['y_pred']) for fold in all_folds_results)
        print(
            f"{'parse_failures':>20}: {total_failures}/{total_samples} "
            f"({(total_failures / total_samples) if total_samples else 0:.1%}) en total"
        )
        print("="*50)
    ...
    def highlight_problematic_sentences(self, texto_original, texto_generado, score=None):
        ...
        resultado = "Coherente" if score == 1 else "No coherente"
