...
def binarize_scores_json(hol_reasons_json, num_annotators):
    """
    Converts the holistic score JSON into a binary score array for each anotator
    """
    json_binary_scores = {}
    ...
    def binarize_score(score):
        if score in [0, 1, 2]:
            ...
            raise ValueError(f"Invalid score {score} for binarization")
    
    for key, value in hol_reasons_json.items():
        json_binary_scores[key] = binarize_score(int(value))
            
                    
    return json_binary_scores
...
def process_dataset(file_path):
    ...
    story_ids = []
    cot_comments = []
    holistic_scores_list = []  # To store holistic scores for each annotator (if needed for analysis)
    ...
    for index, row in data.iterrows():
        ...
        score = binarize_score(int(score))  # Convert to binary score for CoT generation
        ...
        # Holistic data
        holistic_data = row['HolisticData']
        hol_num_anotators = int(holistic_data.get('num_annotators'))
        hol_reasons_json = holistic_data.get('scores', {})
        hol_binary_reason_json = binarize_scores_json(hol_reasons_json, hol_num_anotators)
        
        holistic_scores_list.append(hol_binary_reason_json)  # Store for potential analysis, though not used in CoT generation
    ...
    df = pd.DataFrame({
        ...
        '[score]': scores,
        '[COT]': cot_comments,
        "[num_annotators]": hol_num_anotators,
        "[holistic_scores_per_annotator]": holistic_scores_list  # Same for all rows in this story
    })
...
def balance_target_class(df, target_class=1, target_sample_size=64):
    """
    Downsamples rows with the target class in '[score]' to match a target size (class balancing).
    """
    print(f"Balancing dataset: Downsampling Score {target_class} to {target_sample_size} samples...")
    
    df_target = df[df['[score]'] == target_class]
    df_others = df[df['[score]'] != target_class]

    if len(df_target) > target_sample_size:
        df_target_balanced = df_target.sample(n=target_sample_size, random_state=42)
    else:
        df_target_balanced = df_target
    
    balanced_df = pd.concat([df_others, df_target_balanced], ignore_index=True)
...
def main():
    ...
    if os.path.exists(args.train_input):
        ...
        if train_df is not None:
            ...
            balanced_train_df = balance_target_class(train_df, target_class=1, target_sample_size=84)
